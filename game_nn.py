...
def bird_animation():
    new_bird_rects = [bird_surface.get_rect(center = (100,bird_rect.centery)) for bird_rect in bird_rects]
...
def info_display():
    ...
    for dx,dy in dxy:
        gen_rect = gen_surface.get_rect(midright = (550+dx,100+dy))
        screen.blit(gen_surface, gen_rect)

        num_rect = num_surface.get_rect(midright = (550+dx,150+dy))
        screen.blit(num_surface, num_rect)

        fit_rect = fit_surface.get_rect(midright = (550+dx,200+dy))
        screen.blit(fit_surface, fit_rect)

        score_rect = score_surface.get_rect(center = (288+dx,100+dy))
        screen.blit(score_surface, score_rect)
        
        high_score_rect = high_score_surface.get_rect(center = (288+dx,985+dy))
        screen.blit(high_score_surface, high_score_rect)

        speed_rect = speed_surface.get_rect(midright = (550+dx,250+dy))
    ...
    speed_rect = speed_surface.get_rect(midright = (550,250))
    screen.blit(speed_surface, speed_rect)

def pause_display():
    # darken the screen
    img = pygame.Surface((576,1024))
    img.set_alpha(100)
    screen.blit(img, (0,0))
    
    pause_surface = game_font.render('||', True, (0,0,0))
    for dx,dy in dxy:
        pause_rect = pause_surface.get_rect(center = (288+dx,512+dy))
        screen.blit(pause_surface, pause_rect)
    
    pause_surface = game_font.render('||', True, (255,255,255))
    pause_rect = pause_surface.get_rect(center = (288,512))
    screen.blit(pause_surface, pause_rect)
...
bird_rects = [bird_frames[0][0].get_rect(center = (100,512)) for _ in range(nplayer)]
...
while True:
    ...
    for i in range(nplayer):
        if game_actives[i]:
            ...
            if i == 0: # the best bird of the last generation - red circle
                pygame.draw.circle(screen, (255,0,0), bird_rects[i].center, 50, width=7)
            elif i == 1: # the hatchling of the best bird - yellow cirle
                pygame.draw.circle(screen, (255,255,0), bird_rects[i].center, 50, width=7)
            elif i == nplayer-2: # the worst bird of the last generation - black circle
                pygame.draw.circle(screen, (255,255,255), bird_rects[i].center, 50, width=7)
            elif i == nplayer-1: # the hatchling of the worst bird - white circle
                pygame.draw.circle(screen, (0,0,0), bird_rects[i].center, 50, width=7)
    ...
    if any(game_actives):
        ...
        # save the birds
        with open(r'.\data\data', 'wb') as save_file:
